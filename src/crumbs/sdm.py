...
class SDM:
    ...
    def __init__(
        ...
        background_points: float = 30,
        variables: List[str] = ["bio"],
        ...
        self.presence_shapefile = presence_points,
        self.nb_background_points  = nb_background_points,
        self.times_IDs          = times_IDs,
        ...
        v = set(self.variables); v.add('dem')
    ...
    def _train_and_save_classifiers(train_xs, train_y, target_xs, raster_info):
        ...
        with warnings.catch_warnings():
            ...
            for model_name, (model) in class_map.items():
                ...
                accuracy_scores = model_selection.cross_val_score(model,
                                                                  train_xs,
                                                                  train_y,
                                                                  cv=kfold,
                                                                  scoring='accuracy')
                ...
                print('        - trained model will be saved to', model_name + '.joblib')
                dump(model, model_name + '.joblib')

        return None

    def _random_sample_from_masked_array(masked, nb_sample):
    ...
    def _present_geopanda(gdf) -> None :
    ...
    def _to_geopanda_dataframe(shapefile) :
    ...
    def _sample_background(demRaster, nb_sample=30):
    ...
    def _download_chelsa_variables_at_present_time_() -> None :
        """Download chelsa datasets for present century (time ID = 20) in order to fit classifiers"""
        ...
        self.present_time_DEM =  self.crop_dir + '/' + 'CHELSA_TraCE21k_dem_20_V1.0.tif'

        return None

    def _spatial_plot(x, title, timeID, cmap="Blues"):
    ...
    def _drop_ocean_cells(gdf, rasterFile):
        import rasterio
        import numpy as np
    ...
    def _read_and_clean_presence_dataset():
    ...
    def _plot(pa, filename : str = "presences-pseudo-absences.png") -> None:
    ...
    def _build_presence_absence(presence, pseudo_absence, plot=False):
    ...
    def fit_on_present_data() -> None:
        ...
        explanatory_rasters = self._load_explanatory_rasters()
        ...
        proba_rasters, models_map = self._train_and_save_classifiers(train_xs, train_y, target_xs, raster_info, out_dir, persist)

        return None

    def _find_explanatory_rasters(timeID):
        return sorted(glob.glob(self.landscape_dir + '/*_' + str(timeID) +'_*.tif'))

    def _find_digital_elevation_raster(timeID):
        return self.landscape_dir + '/' + 'CHELSA_TraCE21k_dem_' + str(timeID) + '_V1.0.tif'

    def _load_classifiers_and_impute(timeID, target_xs, raster_info):
        ...
        for model_name in class_map.keys():
            ...
            model = load(sef.joblib_dir + '/' + model_name + '.joblib')

            outdir = self.impute_dir + '/' + model_name + '/' + str(timeID)

            impute(target_xs, model, raster_info, outdir=outdir, linechunk=400, class_prob=True, certainty=True)

            imputed_images.append(outdir  +'/' + 'probability_1.tif')

        return imputed_images

    def _ocean_cells_to_nodata(demRaster, rasters) -> None :
    ...
    def _average_and_save_imputed_rasters(timeID, imputed_images) -> None:
    ...
    def load_classifiers_and_extrapolate(timeID) -> None:
        ...
        explanatory_rasters = self._find_explanatory_rasters(timeID)
        ...
        self._average_imputed_rasters(timeID, imputed_images)
